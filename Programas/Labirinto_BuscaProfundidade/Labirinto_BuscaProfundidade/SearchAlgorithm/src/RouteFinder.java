...
public class RouteFinder {
    ...
    public RouteFinder(Map map,int startX,int startY,int targetX,int targetY){
        ...
        map.setPoint(startX,startY,PontoMapa.MAP_PONIT_OCCUPIED);
    }
    ...
    private void printMapWithRoute(){
        ...
        while (!bestList.isEmpty()){
            ...
            bestMap[point.x][point.y].setType(PontoMapa.MAP_PONIT_OCCUPIED);
        }
        ...
        for (int i =0;i<map.mapHeight;i++){
            for (int j = 0;j<map.mapWidth;j++){
                if (bestMap[i][j].getType() == PontoMapa.MAP_PONIT_OCCUPIED){
                    System.out.print("*"+"\t");
                }else if (bestMap[i][j].getType() == PontoMapa.MAP_PONIT_OBSTRUCTION){
                    System.out.print("█"+"\t");
                }else {
                    System.out.print(" "+"\t");
                }
